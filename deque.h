...
private:
    ...
    void Clone(const deque& D);//клонирование дека
    InfoType GetElement(unsigned);//получение информации n-ного с начала элемента
    bool Empty() const;//проверка на пустоту
public:
    const InfoType GetFirst() const;//получение информации, которая хранится в первом элементе
    const InfoType GetLast() const;//получение информации, которая хранится в поседнем элементе
    unsigned GetSize();//получение размера
    ...
    bool PopLast();//удаление последнегоэлемента дека
    void PushFirst(InfoType);//добавление элемента в начало дека
    bool PopFirst();//удаление первого элемента
};
template <class InfoType>
